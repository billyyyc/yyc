(function() {
	...
	var mContentAnimations = [
		/* 第一篇内容介绍的动画 */
		{
			...
			init : function()
			{
				$("#content_img1_1").css({left:"+=100", opacity:0});
				$("#content_img1_3").css({left:"-=100", opacity:0});
				$("#content_img1_2")
					.css("transform", "scale(0.4)")
					.css("opacity", 0);
				$("#content1 > h3").css({top:"-=40", opacity:0});
				$("#content1 > p").css({top:"+=40", opacity:0});
			},
			show : function()
			{
				$("#content_img1_2")
				...
				{
					$("#content_img1_2")
						.css("transitionDuration", 0.02)
						.css("transform", "scale(1)")
						.css("opacity", 1);
					$("#content_img1_1").animate({left:"-=100", opacity:1}, 400, "easeOutCubic");
					$("#content_img1_3").animate({left:"+=100", opacity:1}, 400, "easeOutCubic");
				}, 300);

				setTimeout(function()
				{
					$("#content1 > h3").animate({top:"+=40", opacity:1}, 300);
					$("#content1 > p").animate({top:"-=40", opacity:1}, 300);
				}, 700);
			}
		},
		/* 第二篇内容介绍的动画 */
		{
			...
			init : function()
			{
				$("#content2 > h3, #content2 > p").css({left:"-=200", opacity:0});
				$("#content2 > em").css({left:"+=200", opacity:0});
			},
			show : function()
			{
				$("#content2 > h3, #content2 > p").animate({left:"+=200", opacity:1}, 500, "easeOutCubic");
				$("#content2 > em").animate({left:"-=200", opacity:1}, 500, "easeOutCubic");
			}
		},
		/* 第三篇内容介绍的动画 */
		{
			...
			init : function()
			{
				$("#content3 > h3, #content3 > p").css({left:"+=200", opacity:0});
				$("#content3 > em").css({left:"-=200", opacity:0});
			},
			show : function()
			{
				$("#content3 > h3, #content3 > p").animate({left:"-=200", opacity:1}, 500, "easeOutCubic");
				$("#content3 > em").animate({left:"+=200", opacity:1}, 500, "easeOutCubic");
			}
		},
		/* 第四篇内容介绍的动画 */
		{
	...
	function unInitPage()
	{
		$("#headerWrapper").css("position","");
		$("#headerToplayout").css({position:"", marginTop:""});
		$("#content").css("top", "");
	...
	$(window).bind("resize", function (e) {
        ...
        if (screenWidth <= 800)
        {
        	...
        	$('body').css("transform", "scale(" + (screenWidth / 720) + ")");
        }
        else
        {
